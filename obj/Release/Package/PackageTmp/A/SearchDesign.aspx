<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SearchDesign.aspx.cs" Inherits="Ipong.A.SearchDesign" %>

<!DOCTYPE html>

<html data-ng-app="formApp2" >
<head >
    ...
    <script src="../Script/App6.js"></script>
...
<div id="welcome">
    ...
    <img alt="Principal Photo" <% try
        {
            if ((Session["pic"].ToString() != "") && (Session["pic"] != null))
            { %> src="../<%=Session["pic"].ToString() %>"<%}
        else
        { %>src="../images/shadow_male.jpg"<%}
        }
        catch (Exception ee) { } %> />
    </div>
    <h1>WELCOME <% try { if (Session["coy_name"] != null) { Response.Write(Session["coy_name"].ToString().ToUpper()); } } catch (Exception ee) { }%> </h1>
    <img alt="Einao Solutions" <% try
        {
            if ((Session["logo"].ToString() != "") && (Session["logo"] != null))
            { %> src="<%=Session["logo"].ToString() %>"<%}
        else
        { %>src="../images/einao_logo.png"<%}
        }
        catch (Exception ee) { } %>  style="width: 200px; height: 70px" /><br />
