...
namespace Ipong.A
{
    public partial class u_application_docs : System.Web.UI.Page
    {
        ...
        private void upload_Clicked(object sender, EventArgs e)
        {
            try
            {
                if ((Session["agentType"] != null) && (Session["agentType"].ToString() != "") && (Session["new_ptID"] != null) && (Session["new_ptID"].ToString() != ""))
                {
                    ...
                    if (agentType == "Agent")
                    {
                        doc_path = base.Server.MapPath("~/") + "admin/ag_docz/ag/" + Session["new_ptID"].ToString() + "/";
                        ...
                        if (IsValid && fu_logo_doc.HasFile)
                        {
                            logo_newfilename = Path.Combine(doc_path, fu_logo_doc.FileName.Replace(" ", "_"));
                            fu_logo_doc.MoveTo(logo_newfilename, MoveToOptions.Overwrite);
                        }

                        pic_newfilename = pic_newfilename.Replace(base.Server.MapPath("~/"), "");
                        logo_newfilename = logo_newfilename.Replace(base.Server.MapPath("~/"), "");

                        if ((pic_newfilename != "0") && (logo_newfilename != "0"))
                        {
                            if (t.updateAgProfileDocz(pic_newfilename, logo_newfilename, Session["new_ptID"].ToString()) != "0")
                            {
