...
namespace Ipong.A
{
    public partial class v_bask_dsu : Page
    {
            ...
            if ((Session["pwalletID"] != null) && (Session["pwalletID"].ToString() != ""))
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if ((Session["agentType"] != null) && (Session["agentType"].ToString() != ""))
                    {
                        ...
                        lt_xdi = ret.GetXDisplayItem("ds", adminID, "1", "Used");
                        if (lt_xdi.Count > 0)
                        {
                            Session["used_cnt"] = lt_xdi.Count;
                            foreach (Classes.XObjs.XDisplayItem x in lt_xdi)
                            {
                                lt_pw = t.getStageByUserIDAccDs(x.new_transID, Session["Sys_ID"].ToString());
                                if (lt_pw.Count > 0)
                                {

                                    SortedList<string, string> sl_app_status = new SortedList<string, string>();

                                    sl_app_status = app_status.showDsStatus(lt_pw[0].status, lt_pw[0].data_status);
                        ...
                        if (tm_vcount > 0)
                        {
                            Response.Redirect("v_bask_dsu.aspx");
                        }
                        else
                        {
        ...
        {
            Response.Redirect("v_bask_ds.aspx");
        }
