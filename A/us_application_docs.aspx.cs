...
namespace Ipong.A
{
    public partial class us_application_docs : Page
    {
            ...
            if ((this.Session["agentType"] != null) && (this.Session["agentType"].ToString() != ""))
            {
                ...
                if (this.agentType == "Agent")
                {
                    if (this.Session["c_reg"] != null)
                    {
                        ...
                        this.Session["agentemail"] = this.email;
                        this.Session["mobile"] = this.mobile;
                        this.Session["c_addy"] = this.c_reg.CompanyAddress;
                        this.Session["Sys_ID"] = this.cust_id;
                        this.Session["xid"] = this.c_reg.xid;
                    }
                }
                else
                {
                    ...
                    this.Session["Sys_ID"] = this.cust_id;
                    this.Session["xid"] = this.c_sub.xid;
                }
            }
            if (!base.IsPostBack)
            {
        ...
        private void upload_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    this.agentType = this.Session["agentType"].ToString();
                    if (this.agentType == "SubAgent")
                    {
                        this.doc_path = base.Server.MapPath("~/") + "admin/ag_docz/agsub/" + this.Session["new_ptID"].ToString() + "/";
                        ...
                        if (base.IsValid && this.fu_pic_doc.HasFile)
                        {
                            ...
                            Session["pic"] = "../admin/ag_docz/agsub/" + this.Session["new_ptID"].ToString() + "/" + fu_pic_doc.FileName.Replace(" ", "_");
                        }
                        this.pic_newfilename = this.pic_newfilename.Replace(base.Server.MapPath("~/"), "");
                        if (this.pic_newfilename != "0")
                        {
                            if (this.t.updateSubAgProfileDocz(this.pic_newfilename, this.Session["new_ptID"].ToString()) != "0")
                            {
