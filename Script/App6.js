...
app.config(function ($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider
        ...
        .state('form.detail2', {
            url: '/detail2',
            templateUrl: 'Change_Name_Detail2b2.html'
        })
...
app.controller('formController', function ($scope, $state, $rootScope, $http, $stateParams, $location) {
    ...
    var serviceBase = serviceBaseIpo + 'Handlers/GetEmail.ashx';
    $(document).ready(function () {
    ...
    $scope.add = function () {
        ...
        if ($scope.formData.Searchname == "rtm") {
            serviceBase = serviceBaseDs + 'Handlers/GetData4a.ashx';
            //  serviceBase = 'http://localhost:40768/api/values/GetData2';
        }

        else {
            serviceBase = serviceBaseDs + 'Handlers/GetData5a.ashx';
        ...
        $http({
            ...
            if (response != "null") {
                ...
                if (response.loa_doc != "" || response.loa_doc == "") {
                    $scope.formData.logo_pic = serviceBaseDs + "admin/pt/" + response.loa_doc;
                    $scope.formData.show = true;

                }
                if (response.ns_doc != "" || response.ns_doc == "") {
                    $scope.formData.ns_doc = serviceBaseDs + "admin/pt/" + response.ns_doc;
                    $scope.formData.show2 = true;
                }
                if (response.pd_doc != "" || response.pd_doc == "") {
                    $scope.formData.pd_doc = serviceBaseDs + "admin/pt/" + response.pd_doc;
                    $scope.formData.show3 = true;
                }
                if (response.doa_doc != "" || response.doa_doc == "") {
                    $scope.formData.doa_doc = serviceBaseDs + "admin/pt/" + response.doa_doc;
                    $scope.formData.show4 = true;
                }
                if (response.rep_pic != "" || response.rep_pic == "") {
                    $scope.formData.rep_pic = serviceBaseDs + "admin/pt/" + response.rep_pic;
                    $scope.formData.show5 = true;
                }

                if (response.rep2_pic != "" || response.rep2_pic == "") {
                    $scope.formData.rep2_pic = serviceBaseDs + "admin/pt/" + response.rep2_pic;
                    $scope.formData.show6 = true;
                }

                if (response.rep3_pic != "" || response.rep3_pic == "") {
                    $scope.formData.rep3_pic = serviceBaseDs + "admin/pt/" + response.rep3_pic;
                    $scope.formData.show7 = true;
                }

                if (response.rep4_pic != "" || response.rep4_pic == "") {
                    $scope.formData.rep4_pic = serviceBaseDs + "admin/pt/" + response.rep4_pic;
    ...
    $scope.submitForm = function (isValid) {
        ...
        if ($scope.formData.Searchname == "rtm") {
            ...
            url9 = serviceBaseDs + "Handlers/Save_GenericApplication3.ashx";
        }

        else {

            url9 = serviceBaseDs +  "Handlers/Save_GenericApplication4.ashx";
        }
