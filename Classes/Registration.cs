...
namespace Ipong.Classes
{
    public class Registration
    {
        ...
        public int updateHwalletStatus(string xid)
        {
            string connectionString = this.ConnectXpay();

            SqlConnection connection = new SqlConnection(connectionString);
            SqlCommand command = new SqlCommand("UPDATE hwallet SET product_title='',used_status='Not Used',used_date=''  WHERE xid='" + xid + "'  ", connection);
            connection.Open();
            int succ = command.ExecuteNonQuery();
            connection.Close();
            return succ;
        ...
        public int updateHwalletToUsedStatus(string product_title, string used_date,string xid)
        {
            string connectionString = this.ConnectXpay();

            SqlConnection connection = new SqlConnection(connectionString);
            SqlCommand command = new SqlCommand("update hwallet set product_title=@product_title,used_date='" + used_date + "',used_status='Used' where xid='" + xid + "'  ", connection);
            command.Parameters.AddWithValue("@product_title", product_title);
